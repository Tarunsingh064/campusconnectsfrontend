...
const PostCard = ({ post }) => {
  ...
  const [localPost, setLocalPost] = useState(post); // For optimistic updates
  ...
  const onLikeClick = async (postId) => {
    ...
    const updatedPost = {
      ...
      like_count: localPost.is_liked ? localPost.like_count - 1 : localPost.like_count + 1
    };
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-center mt-3">
            <div className="flex gap-4">
              <button 
      onClick={() => onLikeClick(localPost.id)}
      disabled={loadingLike}
      className={`flex items-center gap-1 text-sm ${loadingLike ? 'opacity-50 cursor-not-allowed' : 'hover:text-white'} transition text-gray-300`}
    >
      {/* Heart icon with loading spinner fallback */}
      {loadingLike ? (
        ...
        <svg 
          ...
          className={`h-5 w-5 ${localPost.is_liked ? 'text-red-500 fill-current' : 'text-gray-300'}`}
          viewBox="0 0 20 20" 
          fill="none"
          stroke="currentColor"
          strokeWidth={localPost.is_liked ? 0 : 1.5}
        >
          <path 
            strokeLinecap="round" 
      ...
      <span>{localPost.like_count}</span>
    </button>
